...
import io.github.morgaroth.gdrivesync.api.{Auth, GoogleDrive}
...
trait Logic extends hashable {
  ...
  def synchronizeDrive(parent: File) = {
    serviceForDirectory(parent.getAbsolutePath).map(new GoogleDrive(_)) foreach { remote =>
      syncFolder(remote.rootGFile, parent, State(remote.rootGFile, parent, IndexedSeq("/")))
    }
  }
